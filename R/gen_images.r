...
ggplot(subset(data, Type != "Qburst"), aes(peak.value / 1e6, fill=..density.. )) +    
  geom_histogram(alpha = 1, color = "black", aes(y=((..count..)/total_count)), size=1.1, bins = 20) + 
  ...
  labs( x = expression(bold("Peak Value Millions " ~ (pT))), y = expression(bold("Relative Frequency"))) + theme(
...
start_path = "Ind_1/";
ggplot(subset(data, Type != "none"), aes(T1, fill=..ndensity.. )) +    
  geom_histogram(alpha = 1, color = "black", aes(y=after_stat(count) / total_count), size=1.1, bins = 20) + 
  #stat_density(col= "black", fill = "lightblue", alpha=0, size = 1.5, kernel = "gaussian", bw = 0.02) +
  scale_fill_gradient("Count", low="lightblue", high="steelblue1") + 
  labs( x = expression(bold("Rise Time" ~ (s))), y = expression(bold("Relative Frequency"))) + theme(
    plot.title = element_text(color="black", size=14, face="bold.italic", hjust = 0.5),
    axis.title.y = element_text(color="black", size=14, face="bold"),
    axis.title.x = element_text(color="black", size=14, face="bold"),
...
######
start_path = "Ind_1/";
ggplot(subset(data, Type != "Qburst"), aes(PSD.in.band * 1e-10, fill=..density.. )) +    
  geom_histogram(alpha = 1, color = "black", aes(y=..count../total_count), size=1.1, bins = 30) + 
  #stat_density(col= "black", fill = "lightblue", alpha=0, size = 1.5, kernel = "gaussian", bw = 0.1) +
  scale_fill_gradient("Count", low="lightblue", high="steelblue1") + 
  labs( x = expression(bold("Band Power - First SR" ~ (1e10 ~ pT^2))), y = expression(bold("Relative Frequency"))) + theme(
    plot.title = element_text(color="black", size=14, face="bold.italic", hjust = 0.5),
    axis.title.y = element_text(color="black", size=14, face="bold"),
    axis.title.x = element_text(color="black", size=14, face="bold"),
    axis.text.x = element_text(angle=0, size = 12, color = "black"),
